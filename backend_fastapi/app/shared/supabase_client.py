...
class SupabaseClient:
    ...
    def get_session(self, session_id: UUID) -> Optional[dict[str, Any]]:
        ...
        """
        query = self.client.table("sessions").select("*").eq("id", str(session_id))
    ...
    def delete_session(self, session_id: UUID) -> bool:
        ...
        """
        response = (
            self.client.table("sessions").delete().eq("id", str(session_id)).execute()
        )
        return len(response.data) > 0 if response.data else False
    ...
    def create_session_asset(
        ...
        if asset_type not in ("audio", "video", "frame"):
            raise ValueError(
                f"asset_type must be 'audio', 'video', or 'frame', got '{asset_type}'"
            )
        ...
        response = self.client.table("session_assets").insert(asset_data).execute()
    ...
    def delete_session_assets(
        self, session_id: UUID, asset_type: Optional[str] = None
    ) -> int:
        """
    ...
    def _get_cdn_url(self, url: str, settings: Any = None) -> str:
        """
        Convert Supabase Storage URL to CDN URL if CDN is enabled.
        
        Args:
            url: Original Supabase Storage URL
            settings: Settings object with CDN configuration (optional)
            
        Returns:
            CDN URL if enabled, otherwise original URL
        """
        if not settings:
            from ..shared.config import get_settings
            settings = get_settings()
        
        if not settings.cdn_enabled or not settings.cdn_url:
            return url
        
        # Replace Supabase Storage domain with CDN domain
        # Example: https://<project>.supabase.co/storage/v1/object/public/...
        # Becomes: https://cdn.example.com/...
        if "supabase.co/storage" in url:
            # Extract the path after /object/public/
            path_start = url.find("/object/public/")
            if path_start != -1:
                path = url[path_start + len("/object/public/"):]
                return f"{settings.cdn_url.rstrip('/')}/{path}"
    ...
    def upload_file(
        ...
        if hasattr(response, "error") and response.error:
            raise Exception(
                f"Failed to upload file to {bucket_name}/{file_path}: {response.error}"
            )
    ...
    def get_signed_url(
        ...
        if hasattr(response, "error") and response.error:
            raise Exception(
                f"Failed to create signed URL for {bucket_name}/{file_path}: {response.error}"
            )
        elif isinstance(response, dict) and response.get("error"):
            raise Exception(
                f"Failed to create signed URL for {bucket_name}/{file_path}: {response['error']}"
        ...
        if hasattr(response, "data") and isinstance(response.data, dict):
            ...
            return response.get(
                "signedURL", response.get("data", {}).get("signedURL", "")
            )
        elif isinstance(response, str):
            return response

        raise Exception(
            f"Unexpected response format from create_signed_url: {type(response)}"
        )
    ...
    def upload_audio(self, file_data: bytes, filename: str) -> str:
        ...
        Returns:
            Signed URL for the uploaded audio file (CDN URL if enabled)
        """
        file_path = f"audio/{filename}"
        self.upload_file("audio", file_path, file_data, content_type="audio/wav")
        signed_url = self.get_signed_url("audio", file_path)
        return self._get_cdn_url(signed_url)
    ...
    def upload_video(self, file_data: bytes, filename: str) -> str:
        ...
        Returns:
            Signed URL for the uploaded video file (CDN URL if enabled)
        """
        file_path = f"video/{filename}"
        self.upload_file("video", file_path, file_data, content_type="video/mp4")
        signed_url = self.get_signed_url("video", file_path)
        return self._get_cdn_url(signed_url)
    ...
    def upload_frame(self, file_data: bytes, filename: str) -> str:
        ...
        Returns:
            Signed URL for the uploaded frame file (CDN URL if enabled)
        """
        file_path = f"frames/{filename}"
        self.upload_file("frames", file_path, file_data, content_type="image/png")
        signed_url = self.get_signed_url("frames", file_path)
        return self._get_cdn_url(signed_url)
    ...
    def delete_file(
        ...
        if hasattr(response, "error") and response.error:
            raise Exception(
                f"Failed to delete file from {bucket_name}/{file_path}: {response.error}"
            )
        elif isinstance(response, dict) and response.get("error"):
            raise Exception(
                f"Failed to delete file from {bucket_name}/{file_path}: {response['error']}"
        ...
        while True:
            ...
            for asset in assets:
                    ...
                    try:
                        ...
                        if not bucket_name or not file_path:
                            ...
                            if "/" in parsed.path:
                                ...
                                folder_name = (
                                    asset_type if asset_type != "frame" else "frames"
                                )
                                ...
                                raise Exception(
                                    f"Could not parse file path from URL: {asset_url}"
                                )
                        ...
                        error_msg = (
                            f"Failed to delete storage for asset {asset_id}: {str(e)}"
                        )
                ...
                try:
                    ...
                    error_msg = (
                        f"Failed to delete database row for asset {asset_id}: {str(e)}"
                    )
                    stats["errors"].append(error_msg)
    ...
    # ============================================================================
    # Maestro Mode helpers
    # ============================================================================

    def get_maestro_opted_in_user_ids(self) -> list[UUID]:
        """
        Return all user IDs that have explicitly opted into Maestro nudges.

        This powers the nightly aggregation job so we only compute summaries for
        caregivers who enabled the feature in settings.
        """
        response = (
            self.client.table("notification_preferences")
            .select("user_id")
            .eq("maestro_nudges_enabled", True)
            .execute()
        )
    ...
    def create_moderation_item(
        ...
        response = (
            self.client.table("moderation_queue").insert(moderation_data).execute()
        )
    ...
    def resolve_moderation_item(
        ...
        if resolution not in ("resolved", "rejected"):
            raise ValueError(
                f"resolution must be 'resolved' or 'rejected', got '{resolution}'"
            )
