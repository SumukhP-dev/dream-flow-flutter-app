...
void main() {
  group('Story Model Shape Tests', () {
    test('Verify GPT-2 Tiny model input/output shapes', () async {
      ...
      final modelPath = 'android/app/src/main/offloaded_models/gpt2_tiny.tflite';
  ...
  group('Image Model Shape Tests', () {
    test('Verify text encoder model shapes', () async {
      final modelPath = 'android/app/src/main/offloaded_models/sd_text_encoder.tflite';
      if (!await File(modelPath).exists()) {
        print('⚠️ Model file not found: $modelPath');
        print('   Skipping test - bundle model files first');
        return;
    ...
    test('Verify UNet model shapes', () async {
      ...
      try {
        ...
        expect(inputShape.length, equals(4), 
            reason: 'Expected 4D latent tensor [batch, channels, height, width]');
        expect(inputShape[1], equals(4), 
            reason: 'Expected 4 latent channels');
        ...
        expect(outputShape[1], equals(4), 
            reason: 'Expected 4 output channels');
    ...
    test('Verify VAE decoder model shapes', () async {
      ...
      try {
        ...
        expect(outputShape[1], equals(3), 
            reason: 'Expected 3 RGB channels');
        expect(outputShape[2], equals(384), 
            reason: 'Expected height of 384');
        expect(outputShape[3], equals(384), 
            reason: 'Expected width of 384');
