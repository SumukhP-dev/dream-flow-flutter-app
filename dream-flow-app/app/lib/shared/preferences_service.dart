...
class PreferencesService {
  ...
  static const String _keyCaregiverFlag = 'isCaregiver';
  static const String _keyMaestroOptIn = 'maestroOptIn';
  static const String _keyNeuroProfile = 'neuroProfile';
  ...
  /// Determine whether the current user has at least one child profile (caregiver).
  Future<bool> isCaregiver({bool forceRefresh = false}) async {
    final prefs = await SharedPreferences.getInstance();
    if (!forceRefresh) {
      final cached = prefs.getBool(_keyCaregiverFlag);
      if (cached != null) return cached;
    }

    if (!_isLoggedIn || _userId == null) {
      await prefs.setBool(_keyCaregiverFlag, false);
      return false;
    }
    ...
    try {
      ...
      final isCaregiver = data != null;
      await prefs.setBool(_keyCaregiverFlag, isCaregiver);
      return isCaregiver;
    } catch (_) {
      return prefs.getBool(_keyCaregiverFlag) ?? false;
    }
  }

  /// Check whether the caregiver opted into Maestro nudges.
  Future<bool> isMaestroOptedIn({bool forceRefresh = false}) async {
    final prefs = await SharedPreferences.getInstance();
    if (!forceRefresh) {
      final cached = prefs.getBool(_keyMaestroOptIn);
      if (cached != null) return cached;
    }

    if (!_isLoggedIn || _userId == null) {
      await prefs.setBool(_keyMaestroOptIn, false);
      return false;
    }
    ...
    try {
      final response = await Supabase.instance.client
          ...
          .select('maestro_nudges_enabled')
          .eq('user_id', _userId!)
          .maybeSingle();
      final data = (response as dynamic).data;
      final enabled = data != null && (data['maestro_nudges_enabled'] == true);
      await prefs.setBool(_keyMaestroOptIn, enabled);
      return enabled;
    } catch (_) {
      return prefs.getBool(_keyMaestroOptIn) ?? false;
    }
  }

  Future<void> cacheMaestroOptIn(bool enabled) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_keyMaestroOptIn, enabled);
  }

  Future<Map<String, dynamic>?> loadNeuroProfile() async {
    final prefs = await SharedPreferences.getInstance();
    final raw = prefs.getString(_keyNeuroProfile);
  ...
  Future<Map<String, dynamic>?> _getProfile() async {
    ...
    try {
      ...
      try {
        ...
        // Handle other Map types (e.g., Map<dynamic, dynamic>)
        if (data is Map) {
